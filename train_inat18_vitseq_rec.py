...
from data import iNaturalist
from data import FastCollateNeats, create_loader
...
group.add_argument('--num-classes', type=int, default=8142, metavar='N',
...
def main():
    ...
    # Create ViT_MG model
    if args.model=='vit_base_seq_patch16_224':
        model = ViTSEQREC(
                        ...
                        drop_rate=0.,
                        drop_path_rate=0.2,
                        pretrained_path=args.initial_checkpoint,
                    )
    elif args.model=='vit_huge_seq_patch14_224':
        model = ViTSEQREC(
                        img_size=args.input_size[-2:],
                        patch_size=14,
                        in_chans=3,
    ...
    if args.torchcompile:
        ...
        _logger.info("Training with model optimized by torch.compile.")
    ...
    # Local iNat18
    data_root = "/mnt/e1e899db-64e7-4653-855e-c0cbffc9c675/data/datasets/inat2018/"
    ann_train_file = data_root + "/train2018_hier.json"
    ann_val_file = data_root + "/val2018_hier.json"
    # HPC
    ...
    dataset_train = iNaturalist(root=data_root, ann_file=ann_train_file, mode="train", full_info=True)
    dataset_eval = iNaturalist(root=data_root, ann_file=ann_val_file, mode="validation", full_info=True)
    ...
    if mixup_active:
        ...
        if args.prefetcher:
            ...
            collate_fn = FastCollateNeats(**mixup_args)
        else:
    ...
    loader_train = create_loader(
        ...
        use_full_taxon=True,    # To return all taxon info
    )
    ...
    train_loss_fn=nn.CrossEntropyLoss()     # FIXME
    train_loss_fn = train_loss_fn.to(device=device)
    #validate_loss_fn = SoftTargetCrossEntropy().to(device=device)   # FIXME
    ...
    if utils.is_primary(args) and args.log_wandb:
        if has_wandb:
            wandb.init(project=args.experiment, settings=wandb.Settings(start_method='fork'), config=args)
        else:
            _logger.warning(
...
#                 samples, target = mixup_fn(samples, target)
#         if args.channels_last:
...
#             losses_m.update(loss.item() * accum_steps, samples.size(0))
#         update_sample_count += samples.size(0)
...
#                 losses_m.update(reduced_loss.item() * accum_steps, samples.size(0))
...
def train_one_epoch(
    ...
    for batch_idx, (samples, species_id, genus_id, family_id, division_id) in enumerate(loader):
        ...
        target = torch.cat((genus_id.unsqueeze(1), species_id.unsqueeze(1)), dim=1)
...
def validate(
    ...
    batch_time_m = utils.AverageMeter()
    losses_m = utils.AverageMeter()
    family_acc_m = utils.AverageMeter()
    genus_acc_m = utils.AverageMeter()
    species_acc_m = utils.AverageMeter()
    ...
    with torch.no_grad():
        for batch_idx, (samples, species_id, genus_id, family_id, target_species_prob, target_genus_prob, target_family_prob) in enumerate(loader):
            ...
            target = torch.cat((family_id.unsqueeze(1), genus_id.unsqueeze(1), species_id.unsqueeze(1)), dim=1)
            # target = torch.cat((target, torch.ones((target.shape[0], 1), dtype=target.dtype, device=target.device)), dim=1).to(target.device)
            # print(f"target: {target.size()}")
            target_prob = torch.cat((target_family_prob.unsqueeze(1), target_genus_prob.unsqueeze(1), target_species_prob.unsqueeze(1)), dim=1).cuda()
            # print(f"targets probs: {target_prob.size()}")
            ...
            with amp_autocast():
                pred_symbols, contrast_loss = model(samples, target.T)
                # pred_symbols_prob = pred_symbols_prob.to(torch.float16)
                # print(f"symbols: {pred_symbols.shape}")
                # print(f"output: {pred_symbols_prob.shape}")
                # print(f"target_prob: {target_prob.shape}")
                target_x2 = torch.cat((target, target), dim=1)
                loss = loss_fn(pred_symbols.view(-1, pred_symbols.size(2)), target_x2.contiguous().view(-1)) + contrast_loss

            family_acc1, family_acc5 = utils.accuracy(pred_symbols[:,0,:], target[:,0], topk=(1, 5))
            genus_acc1, genus_acc5 = utils.accuracy(pred_symbols[:,1,:], target[:,1], topk=(1, 5))
            species_acc1, species_acc5 = utils.accuracy(pred_symbols[:,2,:], target[:,2], topk=(1, 5))
            ...
            losses_m.update(reduced_loss.item(), samples.size(0))
            family_acc_m.update(family_acc1.item(), target.size(0))
            genus_acc_m.update(genus_acc1.item(), target.size(0))
            species_acc_m.update(species_acc1.item(), target.size(0))
            ...
            if utils.is_primary(args) and (last_batch or batch_idx % args.log_interval == 0):
                ...
                _logger.info(
                    ...
                    f'family_acc@1 {family_acc_m.val:.3f} ({family_acc_m.avg:.3f})\t'
                    f'genus_acc@1 {genus_acc_m.val:.3f} ({genus_acc_m.avg:.3f})\t'
                    f'species_acc@1 {species_acc_m.val:.3f} ({species_acc_m.avg:.3f})\t'
                )

    metrics = OrderedDict([('loss', loss.item()), ('family_acc', family_acc_m.avg), ('genus_acc', genus_acc_m.avg), ('species_acc', species_acc_m.avg)])
